    ...
    <div className="h-screen w-screen overflow-hidden relative bg-background">
      ...
      <div className="absolute inset-0 z-0 bg-background flex items-center justify-center">
        ...
        {isGenerating ? (
          ...
          <div className="w-full h-full [&>div]:h-full [&>div]:!space-y-0 [&_.aspect-video]:!aspect-auto [&_.aspect-video]:absolute [&_.aspect-video]:inset-0 [&_.aspect-video]:!rounded-none [&_.aspect-video]:!border-none [&_.flex-wrap]:absolute [&_.flex-wrap]:bottom-6 [&_.flex-wrap]:left-1/2 [&_.flex-wrap]:-translate-x-1/2 [&_.flex-wrap]:z-20 [&_.flex-wrap]:bg-background/80 [&_.flex-wrap]:backdrop-blur-xl [&_.flex-wrap]:p-2 [&_.flex-wrap]:rounded-2xl [&_.flex-wrap]:border [&_.flex-wrap]:border-border/30 [&_.flex-wrap]:shadow-2xl">
            <WorldViewer worldId={worldId} assets={worldAssets || undefined} className="w-full h-full" />
          </div>
        ) : (
      ...
      <div className="absolute inset-0 z-10 pointer-events-none">
        ...
        <div className="absolute top-[80px] bottom-6 left-6 w-[380px] pointer-events-auto bg-background/90 backdrop-blur-2xl border border-border/30 rounded-2xl flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-border/20 flex items-center gap-3 bg-background/40">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">3D models & worlds</p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6">
            ...
            >
              ...
              <TabsContent value="text" className="mt-5 space-y-3">
                <Label htmlFor="prompt" className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">
                  Prompt
                </Label>
                <Textarea
                  id="prompt"
              ...
              {/* ─── 3D World ─── */}
              <TabsContent value="world" className="mt-5 space-y-5">
                {/* Quality */}
                  ...
                  >
                    {["text", "image"].map((t) => (
                      <div key={t} className="flex items-center space-x-1.5">
                        ...
                        <Label htmlFor={`world-${t}`} className="text-[12px] cursor-pointer capitalize">
                          {t === "text" ? "Text prompt" : "Image"}
                        </Label>
                      </div>
                    ))}
                  </RadioGroup>
                    ...
                    {imagePreview ? (
                      <div className="space-y-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          className="text-[11px] h-6 border-border/30"
                          onClick={() => { setImageFile(null); setImagePreview(null); }}
                          disabled={isGenerating}
                        >
                          Remove
          ...
          {/* Bottom Action Area */}
          <div className="p-5 border-t border-border/20 bg-background/50 backdrop-blur-md">
            {isGenerating && (
        ...
        {/* Model Actions (Floating Bottom Center) */}
        {modelUrl && mode !== "world" && !isGenerating && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 pointer-events-auto flex items-center gap-2 p-2 rounded-2xl bg-background/80 backdrop-blur-xl border border-border/30 shadow-2xl">
            <Button
              className="h-9 px-4 text-[12px] glow-sm"
            ...
            <Button className="h-9 px-4 text-[12px]" variant="ghost" asChild>
              <a href={savedUrl || modelUrl} download="model.glb">
                <Download className="mr-1.5 h-4 w-4" /> GLB
              </a>
            </Button>
          </div>
        )}
